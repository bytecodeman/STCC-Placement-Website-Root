...
<%  
    ...
    if Operation = "SELECT" then
      if ID <> "NEW" then
        ...
        if IsNumeric(ArithScore) and IsNumeric(AlgScore) and IsNumeric(CollegeScore) then
          ...
          Set rs = Nothing
        elseif ArithScore & AlgScore & CollegeScore <> "" then
  ...
  Sub ShowMessage(ByVal TestType, ByVal TestValue, ByVal eStr)
    if TestType = TestValue then
      if eStr <> "" then
        Response.Write "<h3 class=""red center"">Error(s) Occurred in Record Test Details Edit<br />Check Your Input Values</h3>" & vbNewLine
      elseif Request.ServerVariables("REQUEST_METHOD") = "POST" then
...
<div class="center">
  <%
    ...
    if eStr <> "" then
      Response.Write "<h3 class=""red"">Error(s) Occurred in Record Edit<br />" & eStr & "</h3>" & vbNewLine
    elseif Request.ServerVariables("REQUEST_METHOD") = "POST" then
       Response.Write "<h3 class=""green"">Successful Submission!!!" & "</h3>" & vbNewLine
    end if
  %>
...
<%
  ...
  Response.Write "<th>Algebra</th>" & vbNewLine
  Response.Write "<th>College</th>" & vbNewLine
  Response.Write "</tr>" & vbNewLine
  do while not rs.Eof
    Response.Write "<tr>" & vbNewLine
...
<%
  ...
  Response.Write "<th>WP Score</th>" & vbNewLine
  ...
  do while not rs.Eof
    ...
    if IsNull(WPScore) then
      WPScore = "WritePlacer Essay Not Taken"
...
<%
  Call ShowMessage(TestType, "Essay Samples", eStr)
%>
...
<%
  ...
  Response.Write "<input type=""submit"" name=""Operation"" value=""Delete"" style=""margin-right: 25px"" />"
  Response.Write "<input type=""reset"" name=""Reset"" />"
  Response.Write "<input type=""hidden"" name=""TestType"" value=""Essay Samples"" />" 
  Response.Write "</td>" & vbNewLine
  Response.Write "</tr>" & vbNewLine  
  rs.Close
  Set rs = Nothing
