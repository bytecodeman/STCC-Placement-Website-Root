...
<div class="center">
<%
  Call MakeHeader("Upload Placements Failure Report")
  if ErrMsg <> "" then
    Response.Write "<h3 class=""red center"">Error(s) Occurred in Submission<br />" & ErrMsg & "</h3>" & vbNewLine
  end if
%>
<p class="bold">
<a style="margin-right: 10px" href="uploadPlacementDataFailure.asp">Upload Placements Failure Report</a>
...
<table id="report" style="margin-top: 20px;" border="1" cellpadding="2" cellspacing="2">
	<tr>
		<th colspan="4">&nbsp;</th>
		<th colspan="6">Status Values (Click to Remove from Upload)</th>
	</tr>
	<tr>
		<th>Count</th>
		<th>System ID</th>
		<th>SSN/Student ID</th>
		<th>Last Name</th>
		<th>First Name</th>
		<th>Math </th>
		<th>English</th>
	...
	<%
	  ...
	  do while not rs.eof
	    ...
	    Response.Write "<td><a href=""editRecord.asp?value=" & rs("StudentID") & """ target=""_blank"">" & rs("StudentID") & "</a></td>" & vbNewLine
        Response.Write "<td>" & iif(Left(rs("SSN"), 2) = "XX", Right(rs("SSN"), 7), rs("SSN")) & "</td>" & vbNewLine
